...
//  txtHeader->Draw();
	...
	TString file_name = "FitNtuple_Run2012_DoubleMu_v1.root";
	TChain chain_data_dimudimu("FitNtuplePFMuons/dimudimu");
	TChain chain_data_dimuorphan("FitNtuplePFMuons/dimuorphan");
  ...
  // sets of observables (need them for generating toy data)
//  w->defineSet("observables_b1","m_1,m_2");
  
  ostringstream stream_cut_bg_m1;
  stream_cut_bg_m1 << "containstrig2 > 0 && mass > "<< m_min << " && mass < " << m_max;
  
  ostringstream stream_cut_bg_m1_iso;
  stream_cut_bg_m1_iso << "iso_1mm<2. && containstrig2 > 0 && mass > "<< m_min << " && mass < " << m_max;
  
  ostringstream stream_cut_bg_m2;
  stream_cut_bg_m2 << "containstrig > 0 && mass > "<< m_min << " && mass < " << m_max;
  
  ostringstream stream_cut_bg_m2_iso;
  stream_cut_bg_m2_iso << "iso_1mm<2. && containstrig > 0 && mass > "<< m_min << " && mass < " << m_max;
  
  TString cut_bg_m1     = stream_cut_bg_m1.str();
  TString cut_bg_m2     = stream_cut_bg_m2.str();
...
//	TTree* tree_dimuorphan_bg_m1    = chain_data_dimuorphan.CopyTree(cut_bg_m1);
//  TTree* tree_dimuorphan_bg_m2    = chain_data_dimuorphan.CopyTree(cut_bg_m2);
